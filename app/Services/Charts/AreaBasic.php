...
class AreaBasic extends Chart {

	public function render()
	{
        $this->chart->pushSubset('chart', function($title){
        ...
        $this->chart->pushSubset('tooltip', function($title){
            $title->pushCriteria(new Criterias\PointFormat('{series.name} produced <b>{point.y:,.0f}</b><br/>warheads in {point.x}'));
        });

        $this->chart->pushSubset('plotOptions.area', function($title){
            $title->pushCriteria(new Criterias\PointStart(1940));
        });

        $this->chart->pushSubset('plotOptions.area.marker', function($title){
        ...
        $this->chart->pushSubset('plotOptions.area.marker.states.hover', function($title){
